		...
		<form class="form-horizontal" style="margin-top:5px;">
			 <div class="form-group">
			    ...
			    <div class="col-sm-6">
				 ...
				 <div id="messageSpan" class="col-sm-4"></div>
				 <script type="text/javascript">
                     //完成用户名校验的ajax请求
                     //在操作的组件下边完成js代码，无需在加载页面后执行。因为要加载的页面html节点已经加载结束了
                     //1.为节点绑定失去焦点事件
                     $("#username").blur(function() {
                         //2.获取用户输入的内容
                         var username = $(this).val();
                         //3.非空判断，如果为空，直接显示没有输入用户名
                         if(username == "" || username == null) {
                             $("#messageSpan").html("用户名不能为空");
							 $("#registBtn").prop("disabled","disabled");
                             return;
                         }
                         //4.如果不为空，发送ajax请求
                         //4.1准备参数
                         ...
                         $.post(url,param,function(data) {
                             //1.可以使用2不能使用
                             if(data == 1) {
                                 $("#messageSpan").html("恭喜您，这个用户名可以使用");
                                 $("#registBtn").removeAttr("disabled");
                             }else {
                                 $("#messageSpan").html("对不起，这个用户名不能使用");
                                 $("#registBtn").prop("disabled","disabled");
                             }
                         })
                     })
			...
			<label class="radio-inline">
			  ...
			  <div class="form-group">
			    <div class="col-sm-offset-2 col-sm-10">
			      <input id="registBtn" type="submit"  width="100" value="注册" name="submit" border="0"
